...
우리가 찾아야 하는 것은 이 값을 가장 크게 하는 K개의 영벡터가 아닌 직교하는 단위벡터 $$w_k$이다. 
고유분해의 성질로부터 **오른쪽 기저벡터 중 가장 큰 𝐾개의 특잇값에 대응하는 오른쪽 특이벡터가 우리가 찾는 기저벡터가 된다.**
...
이 문제는 원래 행렬 A에 랭크-K 행렬 $$WW^T$$를 곱해서 원래의 행렬 𝐴와 가장 비슷한 행렬 𝐴′을 만드는 문제와 같다
