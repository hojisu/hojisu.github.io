...
- 공분산행렬의 주요 고유벡터는 최대 분산의 방향이 된다. 후속 고유 벡터는 이미 계산 된 벡터와 직교하는 벡터이다. 공분산의 고유벡터는 새로운 feature세트에 대응이 된다(PCA원리)
_________________
...
행렬을 대각화할 수 있으면 **대각화가능(diagonalizable) 행렬**이라고 한다. 고유벡터인 열벡터로 이루어진 행렬에 역행렬이 존재하면 대각화가능이라고 했다. 정방행렬의 역행렬이 존재할 조건은 정방행렬의 열벡터 즉, 고유벡터들이 선형독립인 경우이다. 따라서 행렬이 대각화가능하려면 고유벡터가 선형독립이어야한다.
...
$$
A^{\ast}v^{\ast} = Av^{\ast} = \lambda^{\ast} v^{\ast} 
$$
...
$$
\lambda (v^{\ast})^T v = \lambda^{\ast} v^T v^{\ast} 
$$
​
그런데 $$(𝑣∗)^𝑇𝑣$$도 스칼라이므로
...
행렬 𝑋가 풀랭크이면 𝑋의 열벡터가 **기저벡터**(벡터 N개가 서로 선형독립이고, 이 벡터들을 선형조합하여 만들어지는 모든 벡터의 집합을 벡터공간이고 그 벡터들을 벡터공간의 기저벡터라고 한다.)를 이루기 때문에 영벡터가 아닌 모든 벡터 𝑣에 대해 $$X_v=u$$는 영벡터가 될 수 없다. (만약 영벡터 $$u$$를 만드는 영벡터가 아닌 $$v$$가 존재한다면 서로 독립이 아니다.) 그러면 $$𝑋^𝑇𝑋$$ 의 이차형식은 항상 양수가 된다.
