...
- 벡터 a를 다른 벡터 b에 직교하는 성분과 벡터 b에 평행한 성분으로로 분해할 수 있는데, 평행한 성분을 벡터 b에 대한 투영성분(projection), 벡터 b에 수직인 성분을 벡터 b에 대한 직교성분(rejection)이라고 한다.

______________

### 벡터의 기하학적 의미

N차원 벡터(vector) a는 N차원의 공간상에서 벡터 a의 값으로 표시되는 점(point) 또는원점과 벡터 a의 값으로 표시되는 점을 연결한 화살표(arrow) 이다.
