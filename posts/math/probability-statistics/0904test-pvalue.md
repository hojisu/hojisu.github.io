...
- (모수)검정은 확률분포의 모수값이 어떤 특정한 값을 가진다는 가설을 검정하는 것이다. 
- 검정통계량(test statistics)은 데이터에서 하나로 압축해서 스칼라 값으로 만든 것을 통게량이라고 한다. 이 통계량을 검정에 사용하면 검정통게량이라고 한다. 
- z통계량은 정규분포의 표본평균 통계량이다. 
...
- p-value(유의확률)은 귀무가설이 참일 확률이다. 귀무가설이 검정통계량보다 더 극단적인 값이 나올 확률이다. 만약 p-value가 너무 낮으면, 가설이 일어날 확률이 너무 낮기 때문에 귀무가설을 기각하게 된다. 보통 그 기준은 정하기 나름이지만 일반적인 사회통계학에서는 0.05나 0.01을 기준으로 한다. 이 기준을 바로 유의수준이라고 한다.
- 제 1종 오류는 귀무가설이 참이지만 이를 기각할 확률(alpha), 제 2종 오류는 대립가설이 참이지만 이를 기각할 확률(beta) 이다.
- 검정력(statistical power)은 귀무가설을 올바르게 기각하는 확률입니다. 즉 대립가설이 사실일 때 이를 사실로서 결정할 확률입니다. 검정력이 낮으면 효과를 찾지 못하여 효과기 없다는 잘못된 결론을 내릴 수 있습니다. 검정의 검정력이 너무 높으면 아주 작고 중요하지 않을 수 있는 효과가 유의미하게 보일 수 있습니다. 
_________________
...
검정은 1.모집단의 모수 추정 2. 집단간의 특징 비교 3. 가설 검정을 할 수 있다. 
...
대립가설이 사실일 때, 대립가설을 채택할 확률을 의미합니다. 즉 검정력이란 주장하고자 하는 대립가설이 채택될 확률이 됩니다. 검정력이 낮으면 효과를 찾지 못하여 효과가 없다는 잘못된 결론을 내릴 수 있습니다. 검정의 검정력이 너무 높으면 아주 작고 중요하지 않을 수 있는 효과가 유의하게 보일 수 있습니다. 
검정력이 높을수록 2종 오류를 범할 확률이 낮습니다. 

검정력은 가설 검정을 통해 구한 p-value 값이 0.05보다 크게 나와 통계적으로 유의하지 않을 때 활용합니다. 검정력을 통해 정말로 가설이 무의미한지 수집한 n이 작아 의미가 없어보이는건지를 살펴 볼 수 있습니다. 


![1type-2type-error](../../../resource/img/1type-2type-error)


#### 검정력의 영향요소
1. 유의수준이 높아지면(5% -> 10%) 신뢰도가 나빠져 검정력이 좋아집니다. 
2. 분산 또는 표준편차가 작을수록 검정력이 커진다.
3. 두 모집단 간의 차이가 클수록 검정력이 커진다.
4. 표본 크기가 클수록 검정력이 증가합니다.
5. 효과 크기가 클수록 검정력이 커진다.
...
$$t_0$$ 는 현재 검정통계량의 값이다. 이 유의확률은 통계량분포의 양 끝단의 면적을 구하기 때문에 양측검정 유의확률(two-sided test p-value)이라고 한다.
