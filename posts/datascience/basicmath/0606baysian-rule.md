...
- MAP(maximum a posterior)는 확률을 확률모형으로(사후확률) 모델을 한 후 이 확률모형을 최적화 하는 것입니다. $$\theta$$ 의 분포를 알 수 있기 때문에 분산을 알수 있고 그래서 신뢰도를 알 수 있습니다. $$\theta$$ 를 하나의 값으로 생각하지 않고 $$\theta$$ 에 대한 사전 정보(믿음)을 가지고 모수 $$\theta$$ 에 대해 추정할 수 있다. 베이즈 정리를 이용하여 사후 분포에서 $$\theta$$ 에 대해 극대화 하는 것입니다. $$\arg max_{\theta} \ P(\theta|D) = arg \ max_{\theta} \ \theta^{x+\alpha-1}(1-\theta)^{N-x-\beta-1}$$ 
